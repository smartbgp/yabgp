...
class TestASPATH(unittest.TestCase):
    ...
    def test_parse(self):
        ...
        as_path = ASPath().parse(value='')
        self.assertEqual(as_path, [])

        # 2bytes ASN
        as_path = ASPath().parse(value='\x02\x04\x0c\xb9y3\x88 S\xd9')
        self.assertEqual(as_path, [(2, [3257, 31027, 34848, 21465])])

        # 4bytes ASN
        as_path = ASPath().parse(value='\x02\x04\x00\x00\x0c\xb9\x00\x00y3\x00\x00\x88 \x00\x00S\xd9',
                                 asn4=True)
        self.assertEqual(as_path, [(2, [3257, 31027, 34848, 21465])])

        # MALFORMED_ASPATH
        try:
            ASPath().parse(value='\x02\x04\x00\x00\x0c\xb9\x00\x00y3\x00\x00\x88 \x00\x00S\xd9')
        except UpdateMessageError as e:
            self.assertEqual(e.sub_error, ERR_MSG_UPDATE_MALFORMED_ASPATH)
        try:
            ASPath().parse(value='\x05\x04\x0c\xb9y3\x88 S\xd9')
        except UpdateMessageError as e:
            self.assertEqual(e.sub_error, ERR_MSG_UPDATE_MALFORMED_ASPATH)
    ...
    def test_construct(self):
        ...
        self.assertEqual(as_path, '@\x02\n\x02\x04\x0c\xb9y3\x88 S\xd9')

        # 4 bytes ASN
        as_path = ASPath().construct(asn4=True, value=[(2, [3257, 31027, 34848, 21465])])
        self.assertEqual(as_path, '@\x02\x12\x02\x04\x00\x00\x0c\xb9\x00\x00y3\x00\x00\x88 \x00\x00S\xd9')
