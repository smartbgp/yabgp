...
class TestOriginatorID(unittest.TestCase):
    ...
    def test_parse(self):
        ...
        # normal
        originator_id = OriginatorID().parse(value='\xc0\xa8\x01\x01')
        ...
        self.assertRaises(excep.UpdateMessageError, OriginatorID().parse,
                          '\xc0\xa8\x01\x01\x01')
        try:
            OriginatorID().parse(value='\xc0\xa8\x01\x01\x01')
    ...
    def test_construct(self):
        ...
        self.assertEqual('\x80\x09\x04\xc0\xa8\x01\x01', originator_id)

        # invalid value
        self.assertRaises(excep.UpdateMessageError, OriginatorID().construct,
                          '\xc0\xa8\x01\x01\x01')
        try:
            OriginatorID().construct(value='\xc0\xa8\x01\x01\x01')
        except excep.UpdateMessageError as e:
            self.assertEqual(ERR_MSG_UPDATE_ATTR_LEN, e.sub_error)
